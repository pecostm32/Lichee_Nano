...
extern volatile uint32 usbdisconnect;
...
//Data needs to be aligned on a 32 bit boundary
uint32 massstoragedata[SCSI_BUFFER_SIZE / 4];
        ...
        if(scsi_cbw.signature == MSC_CBW_SIGNATURE)
        {
          ...
          switch(scsi_cbw.command[0])
          {
            ...
            case SCSI_CMD_READ_10:
              ...
              scsi_data_in_ptr = (uint8 *)massstoragedata;
            ...
            case SCSI_CMD_WRITE_10:
              ...
              scsi_data_in_ptr = (uint8 *)massstoragedata;
              scsi_data_end_ptr = scsi_data_in_ptr + sizeof(massstoragedata);
    ...
    case MSC_RECEIVE_DATA:
    {
      ...
      if(tptr < scsi_data_end_ptr)
      {
        ...
        if(length >= scsi_byte_count)
        {
          ...
          if(sd_card_write(scsi_start_lba, sectors, (uint8 *)massstoragedata) != SD_OK)
          {
